...
export async function POST(request: NextRequest) {
  const limited = rateLimit({ request, key: "pm:rewards:claim", limit: 60, windowMs: 60_000 })
  ...
  try {
    ...
    // Claiming moves money: a nonce (replay protection) is always required here.
    ...
    const expectedMessage = buildPmMessage("NoCryCasino PM Rewards Claim v1", {
      wallet_address,
    ...
    if (nonce.length > 0) {
      const used = await consumePmNonce({
        ...
        walletAddress: wallet_address,
        nonce,
        action: "pm_rewards_claim",
        issuedAt: issued_at,
      })
      if (!used.ok) return NextResponse.json({ error: used.error }, { status: used.status })
    }

    const { data, error } = await supabase.rpc("nocry_claim_rewards", { p_wallet: wallet_address })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })

    if (!data?.ok) {
      return NextResponse.json({ error: typeof data?.error === "string" ? data.error : "Claim failed" }, { status: 400 })
    }

    return NextResponse.json({
      ok: true,
      wallet_address,
      claimed_total: Number(data?.claimed_total ?? 0),
      by_mint: data?.by_mint ?? {},
    })
  } catch (e: any) {
